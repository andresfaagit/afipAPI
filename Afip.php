...
class Afip {
	/** File name for the WSDL corresponding to WSAA * @var string **/
	var $WSAA_WSDL;

	/** The url to get WSAA token * @var string **/
	var $WSAA_URL;

	/** File name for the X.509 certificate in PEM format * @var string **/
	var $CERT;

	/** File name for the private key correspoding to CERT (PEM) * @var string **/
	var $PRIVATEKEY;

	/** The passphrase (if any) to sign * @var string **/
	var $PASSPHRASE;

	/** Afip resources folder * @var string **/
	var $RES_FOLDER;

	/** Afip ta folder * @var string **/
	var $TA_FOLDER;

	/** The CUIT to use * @var int **/
	var $CUIT;

	/** Implemented Web Services * @var array[string] **/
	...
	function __construct($options)
		{
			...
			if (!isset($options['res_folder'])) {
				$this->RES_FOLDER = __DIR__.'/Certs/';   // LLEVAR A Certs
			} else {
				$this->RES_FOLDER = $options['res_folder'];
			}

			if (!isset($options['ta_folder'])) {
				$this->TA_FOLDER = __DIR__.'/Ta_tmp/';
			} else {
			...
			if ($options['production'] === TRUE) {
				$this->WSAA_URL = URL_LOG_PROD; //'https://wsaa.afip.gov.ar/ws/services/LoginCms';
			} else {
				$this->WSAA_URL = URL_LOG_HOMO; //'https://wsaahomo.afip.gov.ar/ws/services/LoginCms';
			}
	...
	private function CreateServiceTA($service)
		{
			...
			$client = new SoapClient($this->WSAA_WSDL, array(
				'soap_version'   => SOAP_VERSION, //SOAP_VERSION, //SOAP_1_2,
...
class AfipWebService{
	...
	/** Web service SOAP version * @var intenger **/
	var $soap_version;

	/** File name for the Web Services Description Language * @var string **/
	var $WSDL;
	
	/** The url to web service * @var string **/
	var $URL;

	/** File name for the Web Services Description * Language in test mode * @var string **/
	var $WSDL_TEST;

	/** The url to web service in test mode * @var string **/
	var $URL_TEST;
	
	/** The Afip parent Class * @var Afip **/
	var $afip;
	...
	public function ExecuteRequest($operation, $params = array())
		{
			if (!isset($this->soap_client)) {
				$this->soap_client = new SoapClient($this->WSDL, array(
					'soap_version' 	=> $this->soap_version, //SOAP_VERSION, //|$this->soap_version,
					'location' 		=> $this->URL,
					'proxy_host'	=> PROXY_HOST,
					'proxy_port'	=> PROXY_PORT,
					'trace' 		=> 1,
